...
@st.cache_data
def load_data():
    return pd.read_csv(r'C:\Users\Hamada\Downloads\archive (16)\ai4i2020.csv')
...
if df is not None:
    ...
    fig1 = px.bar(failure_product_ids, x='Product ID', y='Failure Count',
                  ...
                  labels={'Failure Count': 'Failures'}, 
                  color='Product ID')
    st.plotly_chart(fig1)

    # 2. Top 3 Product Types by Failures
    st.header("2. Top 3 Product Types by Count of Machine Failure when it's 1 or Yes Or when there's a Machine Failure")
